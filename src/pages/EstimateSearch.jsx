...
const EstimateSearch = () => {
    ...
    }, [searchQuery, activeFilters, estimates]);
    ...
    const applyFilters = () => {
        ...
        if (searchQuery.trim()) {
            filtered = filtered.filter(est =>
                ...
                est.customer?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                est.customer?.phone_number?.includes(searchQuery) ||
                est.bill_date.includes(searchQuery)
            );
        }
        ...
        if (activeFilters.paymentStatus === 'paid') {
            filtered = filtered.filter(est =>
                est.mode_of_payment === 'cash' || est.mode_of_payment === 'online'
            );
        } else if (activeFilters.paymentStatus === 'unpaid') {
            filtered = filtered.filter(est => est.mode_of_payment === 'unpaid');
        }

        // Customer name filter
        if (activeFilters.customerName.trim()) {
            filtered = filtered.filter(est =>
                est.customer?.name?.toLowerCase().includes(activeFilters.customerName.toLowerCase())
            );
        }
        ...
        if (activeFilters.phoneNumber && activeFilters.phoneNumber.trim()) {
            filtered = filtered.filter(est =>
                est.customer?.phone_number?.includes(activeFilters.phoneNumber)
            );
        }

        // Date range filter
        if (activeFilters.dateFrom) {
            filtered = filtered.filter(est => est.bill_date >= activeFilters.dateFrom);
        }
        if (activeFilters.dateTo) {
            filtered = filtered.filter(est => est.bill_date <= activeFilters.dateTo);
        }

        // Amount range filter
        if (activeFilters.minAmount) {
            filtered = filtered.filter(est =>
                parseFloat(est.total_amount) >= parseFloat(activeFilters.minAmount)
            );
        }
        if (activeFilters.maxAmount) {
            filtered = filtered.filter(est =>
                parseFloat(est.total_amount) <= parseFloat(activeFilters.maxAmount)
            );
        }

        // Payment method filter
        if (activeFilters.paymentMethod) {
            filtered = filtered.filter(est => est.mode_of_payment === activeFilters.paymentMethod);
        }

        // Calculate statistics
        const totalAmount = filtered.reduce((sum, est) => sum + parseFloat(est.total_amount), 0);
        const paidAmount = filtered
            .filter(est => est.mode_of_payment === 'cash' || est.mode_of_payment === 'online')
            .reduce((sum, est) => sum + parseFloat(est.total_amount), 0);
        const unpaidAmount = filtered
            .filter(est => est.mode_of_payment === 'unpaid')
            .reduce((sum, est) => sum + parseFloat(est.total_amount), 0);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    <div className="relative mb-4">
                        ...
                        <input
                            type="text"
                            placeholder="Search by estimate number, customer name, or date..."
                            value={searchQuery}
                                ...
                                <div className="space-y-3 max-h-[600px] overflow-y-auto">
                                    {filteredEstimates.length === 0 ? (
                                        ...
                                        filteredEstimates.map(estimate => (
                                            <div
                                                key={estimate.id}
                                                className={`p-4 border-2 rounded-lg hover:shadow-md transition-all cursor-pointer ${selectedEstimate?.id === estimate.id
                                                    ? 'border-blue-500 bg-blue-50'
                                                    : 'border-gray-200 hover:border-blue-500'
                                                    }`}
                                                onClick={() => navigate(`/billing/estimate/${estimate.id}`)}
                                            >
                                                        ...
                                                        <div className="mt-2 space-y-1">
                                                            ...
                                                            <div className="flex items-center gap-4 text-sm text-gray-600">
                                                                ...
                                                                <span className="flex items-center gap-1 font-semibold">
                                                                    <DollarSign size={14} />
                                                                    ₹{parseFloat(estimate.total_amount).toLocaleString()}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
